...
function inlineCssImports(css, baseUrl) {
  ...
  var _loop = function () {
    ...
    importCssPromises.push(_cssFromImport(importUrl, baseUrl, mediaTypes).then(function (inlinedCss) {
      // now need to rebase all asset url paths inside the resolved imported css
      console.log('rebase css between', { importUrl: importUrl, baseUrl: baseUrl, inlinedCss: inlinedCss });
